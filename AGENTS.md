...
- **`DEMO_MODE=true`** — on startup, if the database has no pets yet (fresh volume after migrations), demo seed runs automatically (append-only; never wipes). Use for PR preview hosts such as `petmon-pr.j0rsa.com` with OIDC (not `DEV_MODE`). Pair with a persistent `DATABASE_URL` volume so re-deploys keep data; wipe the volume to re-seed. PR CI publishes `ghcr.io/j0rsa/petmon:pr-<number>-<run>-<sha>` and triggers Renovate to deploy.
