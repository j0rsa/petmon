...
Ordinary token list/revoke/delete/update operations are owner-scoped. Admin endpoints can inspect, revoke, reactivate and batch-revoke instance credentials after role and scope checks. The inspection endpoint paginates by owner and supports a case-insensitive recorded-name filter, keeping each user's credentials together. An `all` token follows its owner's live administrator role; regular device/MCP tokens never acquire administrative access. Revoked administrator grants take effect on subsequent requests.
...
Resolve timezone from the authenticated actor, not a pet or selected resource: `RuntimeResolver::timezone(pool, actor, fallback)` and `ServiceContext::timezone()`. Standalone always returns the configured instance timezone. Embedded applications can resolve a canonical user's setting, shared across that user's browser/API-token/MCP sessions. APIs, MCP and Shortcuts use the same actor timezone for real-time records, due dates, nutrition status and journal boundaries. A pet selection or ownership change cannot alter the caller's timezone.

Each record stores one UTC RFC3339 instant in `occurred_at` (or weight's `measured_at`), normalized to fixed nanosecond precision and a `Z` suffix for lexical SQL ordering. No parallel UTC fields, source timezone or civil snapshot are stored. APIs require explicit-offset RFC3339 input; frontend forms resolve local civil input in the actor's timezone, reject DST gaps and ask which instant a repeated hour means. Existing clients must update with this release.

Date-only fields and recurring wall-clock schedules retain their civil semantics: birthdays, `local_date` journal days and treatment dates are not timestamps to convert blindly. Editing a journal date alone does not move the instant. Display and chart hours use the current effective actor timezone, while elapsed durations and cutoffs use UTC. A timezone change can change the displayed clock time without rewriting the journal day.

The consolidated `024_embedding_access_control.sql` migration automatically converts the sole pre-release 0.25 data set: naïve May–September 2026 values are Berlin CEST (UTC+2), while explicit-offset values normalize directly. Existing journal dates are preserved. It invalidates classifiers by discarding obsolete model payloads and retaining a `pending_retrain` tombstone rather than adding duplicate timestamp columns. Embedders must ship the same migration; released migrations are unchanged.
...
`useTime()` takes no pet ID and never reads selected-pet state. Standalone `InstanceTimezoneBoundary` loads and validates `/info.timezone` before mounting care forms. Embedded applications supply an already-resolved IANA string in `ApplicationExtensions.timezone` for the current user/session; missing configuration fails closed, without a browser/instance fallback. `now()` injects a test clock. Load the user setting before rendering and change `sessionKey` when identity, timezone or runtime policy changes, so pending requests and drafts cannot retain the previous user's clock.

Worker contexts retain explicit trusted job identities; they do not magically identify a recipient. Standalone workers use the instance timezone. Embedders implementing user-local reminders must orchestrate recipient-specific contexts and delivery/deduplication, recheck authorization, and avoid broadcasting a reminder evaluated in one user's timezone to everybody. Shared classifier time features use UTC consistently because per-pet models must not change with the calling user's timezone.
...
| Weight records, summaries, tags; health-state records | Weight/health services; scoped filters, actor-scoped runtime |
