...
The shortcut picker uses `lines`; each line splits on `|` into label, token, kind, and optional fraction list.
...
| 4 | Parse JSON `lines` array |
| 5 | Multi-select: "Select meds to log" |
| 6 | For each selection: split `\|`, branch on `kind` |
