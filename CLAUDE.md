...
### Device flow contracts (menu response)

AutoMate cannot count a list, index an array of objects, or dedupe strings, so the server hands it values it can compare. These parts of the menu response exist for that and must not regress — the Cherri shortcut no longer needs them (it reads `choices` directly), but the Android flow does:

- **`status`** (`ok` | `empty`) on `GET /shortcuts/meds/intake/menu` — the "nothing due today" branch.
- **Unique `labels`** — a flow carries a chosen label out of the picker and finds its entry by string equality. `shortcut_menu::disambiguate_labels` appends ` (2)`, ` (3)`, … so two meds with the same name+dose can't log one tap twice.
- **`lines`** — pipe-encoded `label|token|kind[|fraction_labels]`, for engines that can only split strings.
- **`fraction_labels`** — display spellings (`1`, `3/4`, `1/2`, …) that a dose picker echoes straight back as `?dose_fraction=`. `DoseFraction` therefore parses both those and the canonical names (`three_quarter`); `DoseFraction::label()`'s `½` glyphs are for the web UI only and do **not** survive a query param. MCP schemas keep the canonical enum.
