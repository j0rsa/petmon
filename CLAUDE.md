...
|------|-------|----------|----------|----------|----------|
| **System / Instance** | `app_settings` | `key` (string) | Shared infrastructure — administered by the operator | OIDC/Telegram REST settings require instance-admin capability; other keys use startup/internal configuration | OIDC config, Telegram bot token, VAPID keys, demo mode |
...
`mcp` intentionally enables care reads and writes through MCP. Ordinary `all` and legacy empty scopes never grant API-token administrator capability. Mint/update/reactivate must attenuate effective scopes, and token ownership is server-assigned. Instance administration requires a live grant plus explicit token `instance_admin` capability (interactive admin sessions derive it from their grant); use shared `auth::admin` checks, including the MCP token-scope tool.

All notification producers use the context's `NotificationBackend`, including request-triggered classification errors and workers. The backend owns persistence/retained ownership, read/count/dismiss and current recipients. Audience errors deny delivery. Standalone explicitly retains its shared broadcast/dismiss behavior. Worker runtimes resolve each pet's timezone and use the injected clock; never copy a global timezone into an embedded job.

New record writes persist canonical UTC instants plus `source_timezone` alongside compatibility civil timestamps. Preserve journal/date-only fields. Legacy backfill is explicit through `record_time::backfill_legacy`, never guessed at startup; ambiguous/nonexistent local times require correction or explicit offsets. External Telegram edits/deletes use persisted original bot/chat/thread context; unknown legacy delivery context must be skipped safely, not redirected to current settings.
