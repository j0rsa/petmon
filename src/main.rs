...
use catmon::{api, assets, config, db, mcp};
use tracing_subscriber::{fmt, EnvFilter};

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    fmt()
        .with_env_filter(EnvFilter::from_default_env().add_directive("catmon=info".parse()?))
        .json()
        .init();

    let config = config::Config::from_env();
    tracing::info!(host = %config.host, port = config.port, timezone = %config.timezone, "starting catmon");
