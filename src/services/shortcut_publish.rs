...
pub fn resolve_med_intake_icloud_url() -> Option<String> {
    ...
    let embedded = ShortcutPublishAssets::get("publish.json")?;
    let cfg: PublishConfig = serde_json::from_slice(&embedded.data).ok()?;
    cfg.icloud_url.and_then(|url| normalize_icloud_url(&url))
}
